...
KBASE_OVERLAY_CHOICES = ['none', 'turnover_extreme', 'turnover_calm', 'turnover_mid']
KBASE_MARKET_REGIME_CHOICES = ['none', 'active_cap_topn']
KBASE_OVERLAY_FEATURES = [
    'turnover_ratio_20', 'turnover_to_60d_max',
    'volume_to_60d_max', 'turnover_extreme_score',
]
...
def _compute_extra(df):
    ...
    df['vol_max60'] = df['volume'].rolling(60, min_periods=1).max()
...
def _extract_features(result, idx):
    ...
    f['turnover_ratio_20'] = _safe_ratio(row.get('turnover', 0), row.get('turnover_ma20', 0))
    f['turnover_to_60d_max'] = _safe_ratio(row.get('turnover', 0), row.get('turnover_max60', 0))
    f['volume_to_60d_max'] = _safe_ratio(row.get('volume', 0), row.get('vol_max60', 0))
    f['turnover_extreme_score'] = (
        0.50 * min(max(f['turnover_ratio_20'] / 2.0, 0.0), 1.0)
        + 0.30 * min(max(f['turnover_to_60d_max'], 0.0), 1.0)
        + 0.20 * min(max(f['volume_to_60d_max'], 0.0), 1.0)
    )
...
def collect_signals(args_tuple):
    csv_path, bullish_dates, pre_start, start_date, end_date, entry_ma_source = args_tuple
    ...
    while i < n:
        ...
        # Overnight features — use T+1 open, NOT T+1 close
        ...
        if entry_ma_source == 't0':
            # Use signal-day indicators (T+0)
            e_yellow = float(result.iloc[i]['yellow_line'])
            e_ma5 = float(result.iloc[i]['ma5'])
        else:
            # Recompute yellow_line and MA5 at T+1 using T+1 OPEN instead of T+1 close.
            # Original MA_N = avg of last N closes (includes T+1 close).
            # Adjusted MA_N = avg of last N-1 closes + T+1 open.
            #               = MA_N_raw + (open - close) / N
...
def apply_kbase_overlay(df, overlay='none', weight=0.15,
                        active_cap_max=None, active_cap_path=ACTIVE_CAP_PATH):
    """Apply optional KBase-inspired pre-entry score overlays."""
    if overlay == 'none':
        return df
    if overlay not in KBASE_OVERLAY_CHOICES:
        raise ValueError('unsupported KBase overlay: {}'.format(overlay))
    weight = max(0.0, min(float(weight), 1.0))
    df = df.copy()
    df['base_score'] = df['score']

    if overlay in {'turnover_extreme', 'turnover_calm', 'turnover_mid'}:
        required = ['turnover_extreme_score', 'turnover_to_60d_max', 'volume_to_60d_max']
        for col in required:
            if col not in df.columns:
                df[col] = 0.0
        base_rank = df.groupby('entry_date')['base_score'].rank(pct=True, method='average')
        turnover_rank = df.groupby('entry_date')['turnover_extreme_score'].rank(pct=True, method='average')
        turn_max_rank = df.groupby('entry_date')['turnover_to_60d_max'].rank(pct=True, method='average')
        vol_max_rank = df.groupby('entry_date')['volume_to_60d_max'].rank(pct=True, method='average')
        df['kbase_overlay_score'] = (
            0.60 * turnover_rank.fillna(0.5)
            + 0.25 * turn_max_rank.fillna(0.5)
            + 0.15 * vol_max_rank.fillna(0.5)
        )
        if overlay == 'turnover_calm':
            df['kbase_overlay_score'] = 1.0 - df['kbase_overlay_score']
        elif overlay == 'turnover_mid':
            raw_mid = 1.0 - (df['turnover_extreme_score'].fillna(0.0) - 0.65).abs() / 0.65
            df['_kbase_mid_raw'] = raw_mid.clip(lower=0.0, upper=1.0)
            df['kbase_overlay_score'] = df.groupby('entry_date')['_kbase_mid_raw'].rank(
                pct=True, method='average'
            ).fillna(0.5)
        blended_score = (1.0 - weight) * base_rank.fillna(0.5) + weight * df['kbase_overlay_score']
        if active_cap_max is not None:
            original_index = df.index
            base_values = base_rank.fillna(0.5).to_numpy()
            blended_values = blended_score.to_numpy()
            merge_col = 'signal_date' if 'signal_date' in df.columns else 'entry_date'
            df[merge_col] = pd.to_datetime(df[merge_col])
            regime = load_active_cap_regime(active_cap_path)
            df = df.merge(regime, left_on=merge_col, right_on='date', how='left').drop(columns=['date'])
            if len(df) == len(original_index):
                df.index = original_index
            df['market_active_cap_ratio20'] = df['market_active_cap_ratio20'].fillna(1.0)
            eligible = df['market_active_cap_ratio20'] <= float(active_cap_max)
            df['score'] = base_values
            df.loc[eligible, 'score'] = blended_values[eligible.to_numpy()]
        else:
            df['score'] = blended_score
        return df
...
def load_active_cap_regime(path=ACTIVE_CAP_PATH):
    df = pd.read_csv(path)
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df = df.dropna(subset=['date']).sort_values('date')
    df = df.drop_duplicates('date', keep='last')
    cap = pd.to_numeric(df['active_cap'], errors='coerce')
    ma20 = cap.rolling(20, min_periods=5).mean()
    ratio = cap / ma20.replace(0, np.nan)
    df['market_active_cap_ratio20'] = ratio.fillna(1.0)
    return df[['date', 'market_active_cap_ratio20']]


def apply_market_regime_topn(df, top_n, mode='none', path=ACTIVE_CAP_PATH):
    if mode == 'none':
        return rank_and_trim(df, top_n)
    if mode != 'active_cap_topn':
        raise ValueError('unsupported market regime mode: {}'.format(mode))
    df = df.copy()
    merge_col = 'signal_date' if 'signal_date' in df.columns else 'entry_date'
    df[merge_col] = pd.to_datetime(df[merge_col])
    regime = load_active_cap_regime(path)
    df = df.merge(regime, left_on=merge_col, right_on='date', how='left').drop(columns=['date'])
    df['market_active_cap_ratio20'] = df['market_active_cap_ratio20'].fillna(1.0)

    kept = []
    for _, grp in df.groupby('entry_date'):
        ratio = float(grp['market_active_cap_ratio20'].iloc[0])
        if ratio < 0.85:
            day_top_n = max(1, int(np.ceil(top_n * 0.4)))
        elif ratio < 1.00:
            day_top_n = max(1, int(np.ceil(top_n * 0.7)))
        else:
            day_top_n = top_n
        ranked = grp.sort_values('score', ascending=False).head(day_top_n).copy()
        ranked['_dynamic_top_n'] = day_top_n
        kept.append(ranked)
    if not kept:
...
def main():
    ...
    ap.add_argument('--output-dir', type=str, default=str(DEFAULT_OUTPUT_DIR),
                    help='Directory for generated research trades/NAV CSV files')
    ...
    ap.add_argument('--kbase-overlay', type=str, default='none', choices=KBASE_OVERLAY_CHOICES,
                    help='Optional AG2-KBase pre-entry score overlay')
    ap.add_argument('--kbase-overlay-weight', type=float, default=0.15,
                    help='KBase overlay weight in final daily rank score (default 0.15)')
    ap.add_argument('--kbase-overlay-active-cap-max', type=float, default=None,
                    help='Apply KBase overlay only when signal-day active_cap/MA20 <= this value')
    ap.add_argument('--kbase-market-regime', type=str, default='none',
                    choices=KBASE_MARKET_REGIME_CHOICES,
                    help='Optional AG2-KBase market regime risk control')
    ...
    ap.add_argument('--entry-ma-source', type=str, default='t0', choices=['t0', 't1_open'],
                    help='t0=signal-day MA, t1_open=T+1 open-adjusted MA (default)')
    args = ap.parse_args()
    ...
    pool_args = [(str(f), bullish_dates, pre_start, start_date, end_date, args.entry_ma_source) for f in csv_files]
    all_sigs = []
    done = 0
    total = len(csv_files)
    t0 = datetime.now()
    ...
    if not all_sigs:
        if args.save_raw:
            empty_cols = [
                ...
                *FEATS_ML, *KBASE_OVERLAY_FEATURES,
            ]
            pd.DataFrame(columns=empty_cols).to_csv(args.save_raw, index=False, encoding='gbk')
            print('No signals! Empty raw signal file saved: {}'.format(args.save_raw))
            return
        print('No signals!')
        return
    ...
    if args.save_raw:
        ...
        df_raw.to_csv(args.save_raw, index=False, encoding='gbk')
        print('Raw signals saved: {} ({})'.format(args.save_raw, len(df_raw)))
        return
    ...
    if args.kbase_overlay != 'none':
        before_score = df_scored['score'].copy()
        before_rank = before_score.groupby(df_scored['entry_date']).rank(
            ascending=False, method='first'
        )
        df_scored = apply_kbase_overlay(
            df_scored,
            overlay=args.kbase_overlay,
            weight=args.kbase_overlay_weight,
            active_cap_max=args.kbase_overlay_active_cap_max,
            active_cap_path=ACTIVE_CAP_PATH,
        )
        after_rank = df_scored.groupby('entry_date')['score'].rank(
            ascending=False, method='first'
        )
        changed = (before_rank.to_numpy() != after_rank.to_numpy()).sum()
        print('KBase overlay: {} weight={:.2f}, same-day rank changes={}'.format(
            args.kbase_overlay, args.kbase_overlay_weight, int(changed)))

    df_final = apply_market_regime_topn(
        df_scored,
        args.top_n,
        mode=args.kbase_market_regime,
        path=ACTIVE_CAP_PATH,
    )
    if args.kbase_market_regime != 'none':
        dyn = df_final.get('_dynamic_top_n')
        if dyn is not None and len(dyn) > 0:
            print('KBase market regime: {}, avg dynamic TopN={:.2f}'.format(
                args.kbase_market_regime, dyn.mean()))
    ...
    # Save
    suffix = {'A': 'a', 'B': 'b', 'ml': 'ml'}[args.method]
    suffix += '_' + args.entry_ma_source
    if args.top_n != 3:
        suffix += '_top{}'.format(args.top_n)
    if args.min_score is not None:
        score_tag = str(args.min_score).replace('-', 'm').replace('.', 'p')
        suffix += '_ms{}'.format(score_tag)
    if not use_timing:
        suffix += '_notiming'
    if args.kbase_overlay != 'none':
        suffix += '_kb_{}_w{:02d}'.format(
            args.kbase_overlay,
            int(round(max(0.0, min(args.kbase_overlay_weight, 1.0)) * 100)),
        )
    if args.kbase_overlay_active_cap_max is not None:
        cap_tag = str(args.kbase_overlay_active_cap_max).replace('.', 'p')
        suffix += '_acmax{}'.format(cap_tag)
    if args.kbase_market_regime != 'none':
        suffix += '_mr_{}'.format(args.kbase_market_regime)
...
def build_account_nav(trades, start_date, end_date, args):
    ...
    for code in codes:
        ...
        try:
            ...
            dates = df_p['date'].dt.normalize().to_numpy(dtype='datetime64[ns]')
            price_cache[padded] = dict(zip(df_p['date'].dt.normalize(), df_p['close']))
            price_dates[padded] = dates
        except Exception:
            continue
        ...
        if positions:
            ...
            for p in positions:
                ...
                if dates is not None and len(dates) > 0:
                    ...
                    if prev_idx >= 0:
                        prev = cache.get(pd.Timestamp(dates[prev_idx]).normalize(), None)
                cur = cache.get(cur_date, None)
                if p['entry'] == today and p['exit'] == today:
                    if p['entry_price'] > 0:
                        pos_rets.append((p['exit_price'] - p['entry_price']) / p['entry_price'])
                elif p['entry'] == today:
                    if cur and p['entry_price'] > 0:
                        pos_rets.append((cur - p['entry_price']) / p['entry_price'])
                elif p['exit'] == today:
                    if prev and prev > 0:
                        pos_rets.append((p['exit_price'] - prev) / prev)
    ...
    filename = 'backtest_brick_nav_{}.csv'.format(suffix) if suffix else 'backtest_brick_nav.csv'
    out = _output_path(args, filename)
