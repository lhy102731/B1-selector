...
registry = get_registry(str(project_root / "config" / "strategy_params.yaml"))
...
# 加载策略
registry.auto_register_from_directory(str(project_root / "strategy"))
...
def update_config():
    ...
    try:
        ...
        authorize_mutation(
            _web_auth(),
            operation="WEB_CONFIG_WRITE",
            callable_name="update_config",
            paths=(config_file,),
        )
    ...
    except WebAuthorizationError as e:
        return jsonify({'success': False, 'error': str(e), 'code': 'WEB_AUTH_REQUIRED'}), 403
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
...
if __name__ == '__main__':
    run_web_server(host='127.0.0.1', debug=False)
