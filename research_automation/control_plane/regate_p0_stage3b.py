    ...
    try:
        with patch.multiple(
            ...
            policy_actor = Actor(
                ...
                "automation",
                f"p0-policy-activation-exec-023-{run_suffix}",
            )
            ...
            ticket = authority._issue_task_ticket(
                grant,
                {
                    "task_id": "P0-POLICY-ACTIVATION-023",
                    "objective": "activate reviewed entry policy for p0-attempt-023",
                    "dependencies": [],
                    "idempotency_key": f"p0-policy-activation-023-cr010-{run_suffix}",
