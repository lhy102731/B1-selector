    ...
    try:
        with patch.multiple(
            ...
            policy_actor = Actor(
                ...
                "automation",
                f"p0-policy-activation-exec-027-{run_suffix}",
            )
            ...
            ticket = authority._issue_task_ticket(
                grant,
                {
                    "task_id": "P0-POLICY-ACTIVATION-027",
                    "objective": "activate reviewed entry policy for p0-attempt-027",
                    "dependencies": [],
                    "idempotency_key": f"p0-policy-activation-027-cr010-{run_suffix}",
