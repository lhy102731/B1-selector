    ...
    try:
        with patch.multiple(
            ...
            policy_actor = Actor(
                ...
                "automation",
                f"p0-policy-activation-exec-038-{run_suffix}",
            )
            ...
            ticket = authority._issue_task_ticket(
                grant,
                {
                    "task_id": "P0-POLICY-ACTIVATION-038",
                    "objective": "activate reviewed entry policy for p0-attempt-038",
                    "dependencies": [],
                    "idempotency_key": f"p0-policy-activation-038-cr010-{run_suffix}",
