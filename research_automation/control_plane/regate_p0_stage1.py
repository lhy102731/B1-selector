"""CR-010 P0 re-gate STAGE 1: activate p0-attempt-030 (coordinator only).
...
def main() -> int:
    ...
    env_rel = f"activation-envelopes/p0-gate-030.json"
    ...
    manifest_blob = subprocess.run(
        ["git", "-C", str(ROOT), "cat-file", "blob",
         f"{envelope_commit}:"
         f"research_state/control_plane/p0/attempts/{ATTEMPT}/"
         "activation-envelopes/p0-gate-030.json"],
        capture_output=True,
    ).stdout
